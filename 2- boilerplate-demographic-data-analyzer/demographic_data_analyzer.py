...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round(df.loc[df['education'] == 'Bachelors'].shape[0] / size * 100, 1)

    # What percentage of people with advanced education (`Bachelors`, `Masters`, or `Doctorate`) make more than 50K?
    # What percentage of people without advanced education make more than 50K?

    # with and without `Bachelors`, `Masters`, or `Doctorate`
    higher_education = (df['education'] == 'Masters') | (df['education'] == 'Bachelors') | (df['education'] == 'Doctorate')
    ...
    perces = df.loc[df['salary'] == '>50K', 'native-country'].value_counts() / df['native-country'].value_counts()
    highest_earning = perces.loc[perces == perces.max()]
    highest_earning_country = highest_earning.index[0]
    highest_earning_country_percentage = round(highest_earning[0]*100, 1)
